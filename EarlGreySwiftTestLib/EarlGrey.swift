...
private func GREYAssert(
  ...
  let waitError = GREYWaitUntilIdle()
...
private func GREYWaitUntilIdle() -> NSError? {
  let timeout = GREYConfiguration.shared.doubleValue(forConfigKey: GREYConfigKey.interactionTimeoutDuration)

  var waitError: NSError?
  GREYWaitForAppToIdleWithTimeoutAndError(timeout, &waitError)
  return waitError
}
...
public struct EarlGrey {
  ...
  public static func hostApplicationCrashHandler(_ crashHandler: GREYHostApplicationCrashHandler?) {
    EarlGreyImpl
      .invoked(fromFile: #file, lineNumber: #line)
      .setHostApplicationCrashHandler(crashHandler)
  }
...
public extension GREYInteraction {
  @discardableResult func assert(_ matcher: @autoclosure () -> GREYMatcher) -> Self {
    return __assert(with: matcher())
  }

  @discardableResult func assert(
  ...
  @discardableResult func perform(_ action: GREYAction) -> Self {
  ...
  @discardableResult func usingSearch(
    action: GREYAction,
    onElementWith matcher: GREYMatcher
  ) -> Self {
    return __usingSearch(action, onElementWith: matcher)
  }
}
